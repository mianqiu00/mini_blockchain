...
void writeFilenameToCSV(const char *filename)
{
    ...
    FILE *file = fopen("./message1/block_message1.csv", "a");
...
Block* Add_Block(Block* list)
{
    ...
    char filename[20];
    sprintf(filename, "./message1/block_%d.json", block->height);
...
void loopActivate(Block* list)
{
    ...
    while (status)
    {
        ...
        {
            char* jsonname = getMessage("./message2/block_message2.csv");
            ...
            {
                // 区块消息队列为空，查询客户消息队列
                char* sign = getMessage("./message2/agent_message2.csv");
